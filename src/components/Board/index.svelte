<script>
	...
	function getValueAtCursor(userGrid, cursorStore) {
		if (cursorStore.x === null && cursorStore.y === null) return null;
		return userGrid[cursorStore.y][cursorStore.x];
	}
</script>
...
<div class="board-padding relative z-10">
	...
	<div class="board-padding absolute inset-0 flex justify-center">
		<div class="bg-white shadow-2xl rounded-xl overflow-hidden w-full h-full max-w-xl grid" class:bg-gray-200={$gamePaused}>
			{#each $gameState.userGrid as row, y}
				{#each row as value, x}
					<Cell
						{value}
						cellY={y + 1}
						cellX={x + 1}
						candidates={$gameState.candidates[x + ',' + y]}
						disabled={$gamePaused}
						selected={isSelected($cursor, x, y)}
						userNumber={$gameState.grid[y][x] === 0}
						sameArea={$settings.highlightCells && !isSelected($cursor, x, y) && isSameArea($cursor, x, y)}
						sameNumber={$settings.highlightSame && value && !isSelected($cursor, x, y) && getValueAtCursor($gameState.userGrid, $cursor) === value}
						conflictingNumber={$settings.highlightConflicting && $gameState.grid[y][x] === 0 && $gameState.invalidCells.includes(x + ',' + y)}
					/>
				{/each}
			{/each}
		</div>
